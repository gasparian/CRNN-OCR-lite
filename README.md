...
Why I think so? Actually it's super-hard to get 0% error rate on real-world photos. So if you want to use "hand-made" detection+recognition pipeline to "digitize" text on photos, you'll most likely need to check and correct all recognized words or add non-recognized ones. This is pretty same expirience to the current "pdf-scanners" (which is pain). And on the other side, if the model can detect and recognize even 20% of words on image, you still be able to find something using text search.  
